...
public class MainView {
  ...
  public MainView() {
    ...
    // With 695k scale, celebrities span ~1.4M units - need large far clip
    camera.setNearClip(10.0); // Increased to prevent z-fighting with large distances
    camera.setFarClip(3000000.0); // 3 million - encompasses entire cloud at any zoom level
    camera.setTranslateZ(-1000000); // Start FAR back to show whole cloud initially
    ...
    statusLabel.setStyle(
        "-fx-font-size: 18px; -fx-text-fill: rgba(255,255,255,0.7); -fx-font-weight: bold;");
              ...
              try {
                ...
                Platform.runLater(
                    () -> {
                      ...
                      statusLabel.setText(
                          String.format("Found %d similar celebrities!", matches.size()));
                ...
                Platform.runLater(
                    () -> {
                      ...
                      statusLabel.setStyle(
                          "-fx-font-size: 14px; -fx-text-fill: #ff6b6b; -fx-font-weight: bold;");
                    });
              }
            })
        .start();
  ...
  private void highlightMatches(List<Celebrity> matches, Image droppedImage) {
    ...
    if (droppedImage != null) {
      ...
      System.out.println(
          "DEBUG: Input image at Z=" + inputImageZ + ", pushed all thumbnails behind");
      ...
      droppedImageView.setOnMouseClicked(
          event -> {
    ...
    for (int i = 0; i < connectCount; i++) {
      ...
      for (int j = 0; j < celebrities.size(); j++) {
        if (celebrities.get(j).getCelebrity().getId().equals(match.getId())) {
          ...
          System.out.println(
              "DEBUG: Found match at index " + j + ", applying border and connection");
              ...
              try {
                ...
                Platform.runLater(
                    () -> {
                      ...
                      statusLabel.setText(
                          String.format("%d celebrities loaded", celebrities.size()));
  ...
  private void createLayout() {
    ...
    dropHint.setStyle(
        "-fx-font-size: 18px; -fx-text-fill: rgba(255,255,255,0.3); -fx-font-weight: bold;");
  ...
  private void renderCelebrities() {
    ...
    for (int i = 0; i < celebrities.size(); i++) {
      ...
      imageView.setFitWidth(20); // Very small thumbnails to minimize overlap with 11k+ images
  ...
  /**
   * Create or load celebrity avatar/image. Tries to load actual image from resources first, falls
   * back to placeholder with initials.
   */
  ...
  private void addMouseControls() {
    ...
    scene3D.setOnScroll(
        event -> {
          ...
          // Adaptive zoom speed for large 3D scale - gentle navigation
          // Speed scales with distance but kept conservative for control
          double currentZ = camera.getTranslateZ();
          double zoomSpeed = Math.abs(currentZ) * 0.0002; // 0.02% of current distance
          zoomSpeed = Math.max(100, Math.min(zoomSpeed, 5000)); // Clamp between 100-5k
          camera.setTranslateZ(currentZ + delta * zoomSpeed);
        });
  }

  private void highlightCelebrity(Celebrity celebrity, ImageView clickedView) {
    System.out.println(
        "DEBUG: Clicked celebrity=" + celebrity.getName() + ", id=" + celebrity.getId());

    // Update selected label
    selectedCelebLabel.setText(
        String.format("Selected: %s (ID: %s)", celebrity.getName(), celebrity.getId()));
  ...
  private void centerOnThumbnail(ImageView thumbnail) {
    ...
    javafx.animation.TranslateTransition transition =
        new javafx.animation.TranslateTransition(javafx.util.Duration.seconds(0.5), root3D);
    transition.setToX(-thumbnail.getTranslateX());
    transition.setToY(-thumbnail.getTranslateY());
    transition.play();

    // Zoom in
    javafx.animation.TranslateTransition zoomTransition =
        new javafx.animation.TranslateTransition(javafx.util.Duration.seconds(0.5), camera);
  ...
  private void centerAndBringForward(ImageView target) {
    ...
    javafx.animation.TranslateTransition resetPan =
        new javafx.animation.TranslateTransition(javafx.util.Duration.seconds(0.3), root3D);
    resetPan.setToX(0);
    resetPan.setToY(0);

    // Reset camera zoom to see full cloud
    javafx.animation.TranslateTransition resetZoom =
        new javafx.animation.TranslateTransition(javafx.util.Duration.seconds(0.3), camera);
    resetZoom.setToZ(-2000);

    // Step 2: After reset completes, center on target within viewport
    resetZoom.setOnFinished(
        e -> {
          ...
          javafx.animation.TranslateTransition panTransition =
              new javafx.animation.TranslateTransition(javafx.util.Duration.seconds(0.5), root3D);
          ...
          javafx.animation.ParallelTransition focusTransition =
              new javafx.animation.ParallelTransition(panTransition, zoomTransition);

          focusTransition.play();
        });

    // Start the reset sequence
    javafx.animation.ParallelTransition resetTransition =
        new javafx.animation.ParallelTransition(resetPan, resetZoom);
    ...
    if (selectedCeleb != null) {
      selectedCelebLabel.setText(
          String.format("Selected: %s (ID: %s)", selectedCeleb.getName(), selectedCeleb.getId()));
    }
  }

  /**
   * Calculate how much of the input image is visible from a camera position. Counts thumbnails
   * between camera and input image that would occlude it.
   *
   ...
   */
  private double calculateVisibility(
      ImageView inputImage, List<ImageView> allThumbnails, double cameraZ) {
    ...
    for (ImageView thumb : allThumbnails) {
      ...
      if (thumbZ > cameraZ && thumbZ < inputZ) {
        ...
        double distance2D = Math.sqrt(Math.pow(thumbX - inputX, 2) + Math.pow(thumbY - inputY, 2));
  ...
  @edu.umd.cs.findbugs.annotations.SuppressFBWarnings(
      value = "EI_EXPOSE_REP",
      justification = "JavaFX pattern: root node is intentionally exposed for scene creation")
